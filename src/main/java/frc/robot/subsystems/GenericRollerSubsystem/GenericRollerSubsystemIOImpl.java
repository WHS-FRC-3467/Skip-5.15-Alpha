...
public abstract class GenericRollerSubsystemIOImpl implements GenericRollerSubsystemIO {
  ...
  public GenericRollerSubsystemIOImpl(GenericRollerSubsystemConstants constants, boolean isSim) {
    ...
    for (int i = 0; i < mNumMotors; i++) {
      ...
      TalonFX mMotor =
          new TalonFX(mConst.kMotorIDs.get(i).getDeviceNumber(), mConst.kMotorIDs.get(i).getBus());
      mMotors.add(mMotor);
  ...
  public void updateInputs(GenericRollerIOInputs inputs) {
    ...
    /*
     * Because this code supports a configurable number of motors, we must wait until the first time
     * this method is called to instantiate arrays of the proper size
     */
    if (!mInitialized) {
