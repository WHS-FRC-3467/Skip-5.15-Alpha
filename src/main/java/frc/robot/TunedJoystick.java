...
public final class TunedJoystick {
  ...
  private CommandXboxController cntrllr;
  ...
  public TunedJoystick(CommandXboxController c) {
    this.cntrllr = c;
  ...
  /**
   * Sets the period on which TunedJoystick performs deadzone and scaling operations. Default is
   * 20ms. Only use this if you know what you're doing.
   */
  ...
  void tuneLeftStick() {
    ...
    if (t2 - t1_lft < periodMilliseconds) return;
  ...
  void tuneRightStick() {
    ...
    if (t2 - t1_rght < periodMilliseconds) return;
